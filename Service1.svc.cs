    ...
    public class Routes : IRoutes, IDisposable
    {
        ...
        public string SaveRoute(Route route)
        {
            ...
            if (gpxConnection.IsConnect())
            {
                try
                {
                    ...
                    {
                        query = string.Format("insert into routes (dest,distance,descrip,climbing,route,owner,startplace, starttime) values ('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}')",
                            route.Dest, route.Distance, route.Descrip, route.Climbing, route.GPX, route.Owner,route.Place, route.Time);
        ...
        public IEnumerable<Route> GetRouteSummaries()
        {
            ...
            if (gpxConnection.IsConnect())
            {
                try
                {
                    // get routess associated to that user
                    //string query = string.Format("SELECT * FROM routes where routes.owner = {0}", userID);
                    string query = string.Format("SELECT dest,descrip,distance,climbing,owner FROM routes ");
                    ...
                    using (MySqlDataAdapter routeAdapter = new MySqlDataAdapter(query, gpxConnection.Connection))
                    {
                        ...
                        for (int row = 0; row < length; row++)
                        {
                            ...
                            try
                            {
                                ...
                                routes.Add(new Route(null, dest, descrip, distance, climbing, owner, "", DateTime.MinValue));
                            }
                            catch (Exception ex)
                            {
            ...
            log.Result = routes.Count.ToString() + " routes";
            log.Save(gpxConnection);
            gpxConnection.Close();
            return routes;
        }
